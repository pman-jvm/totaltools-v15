...
dom.query.getJSON($url, function (data) {
  ...
  $merchantId = "";
  if ($el){
  $amount = $el && $el.getAttribute("data-amount") || 0;
  $intRate = data;
  $before = $el && $el.getAttribute("data-before") || "Credit Amount";
  $after = $el && $el.getAttribute("data-after") || "per month";
  ...
  $logoImage =
    $el.getAttribute("data-logoImg") ||
    "https://app.mobicredwidget.co.za/assets/img/logo-mobicred-grey.png";
  ...
  document.getElementById("instalmentCalc").style.display = "inline-block";
  ...
  $apr = $rate / 1200;
  $formula =
    ($apr * -$amount * Math.pow(1 + $apr, $term)) /
  ...
  if ($amount <= $maxAmount) {
    $fullText =
      "When financing " +
      $amountCurrency +
      " through Mobicred over a 12 month period using a " +
      $rate +
      "% interest rate, your monthly instalment price will be " +
      $instalment;
  } else {
    $fullText =
      "When financing " +
      ...
      " through Mobicred over a 12 month period using a " +
      $rate +
      "% interest rate, your monthly instalment cannot be calculated as your Max ceiling has been set at R" +
      $maxAmount;
  }

   //ts modification
  //Console log just for testing
  self.mobicred_isntallment = $instalment;
//  console.log($amount, $maxAmount, $rate, $instalment, $displayText, $fullText);
  ...
  if ($showLogo == 1) {
    $logo =
      "<div class='mobicred-logo'> <a target='_blank' href='https://live.mobicred.co.za/cgi-bin/wspd_cgi.sh/WService=wsb_mcrliv/run.w?run=application&merchantID=" +
      $merchantId +
      "&returnUrl=" +
