...
class StockValuationLayerRevaluation(models.TransientModel):
    ...
    def action_validate_revaluation(self):
        ...
        product_id = self.product_id.with_context(default_company_id=self.company_id.id, force_company=self.company_id.id)
        ...
        if product_id.categ_id.property_cost_method == 'average':
            description += _("Product cost updated from {previous} to {new_cost}.".format(previous=product_id.standard_price,
        ...
        move_vals = {
            ...
            'move_type': 'entry',
            'line_ids': [(0, 0, {
                'name': _('{user} changed stock valuation from  {previous} to {new_value} - {product}'.format(
